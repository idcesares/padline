...
#    To seal what you saw now and decide later (remove, below, captures for you):
node scripts/admin.mjs <host> case <id> capture

# 3. Act, with a reason (required) and optionally the legal basis:
node scripts/admin.mjs <host> case <id> remove --reason "<why>" --legal-basis "Content Policy: <rule>"
#   ^ policy violation: seal evidence, then wipe content + snapshots AND block the slug so it can't be refilled
#     (purging a violation without evidence is refused unless you pass --without-evidence)
...
`node scripts/admin.mjs <host> cases --status open` lists open cases, grave first; `bulk <file> <action> --category <id> --reason "..."` runs one action over a file of slugs or pad URLs, as a dry run until `--apply`; `stats --from YYYY-MM-DD` totals notices, cases, and actions and measures review and action times against the profile's targets; `export reports|cases|actions|evidence --format csv` writes a spreadsheet-safe export (reporter contact only with `--include-contact`, and every export is itself logged), and `verify-chain --file <actions.json>` checks an exported action log offline; `case <id>` shows a case's notices and actions; `evidence <id> download --out <dir>` writes sealed evidence with both hashes verified, and `evidence <id> hold|release --reason` keeps it past the retention period (180 days after its case closes, in the moderation profile); `freeze --reason` keeps a pad readable but refuses every edit while you investigate (`unfreeze` lifts it); `disconnect --reason` drops live connections without changing access; `unblock --reason` reverses a block applied in error; `verify-chain` checks the action log; `reconcile` resolves an action interrupted between the room and the ledger. `<host>` is your domain (e.g. `padline.page`) or `127.0.0.1:8788` locally.
...
| `node scripts/admin.mjs <host> <command>` | Moderation CLI over the ledger: `cases`, `case open`, `case <id> review\|capture\|freeze\|unfreeze\|disconnect\|block\|unblock\|purge\|dismiss\|close`, `case <id> remove`, `evidence <id> [download\|hold\|release]`, `bulk`, `stats`, `export`, `pad <slug> info`, `reconcile`, `verify-chain` |
